...
db.products.createIndex({ id_product: 1 }, { unique: true });
db.product_variants.createIndex({ id_variant: 1 }, { unique: true });
db.gear_specifications.createIndex({ id_product: 1 }, { unique: true });
...
db.products.createIndex({ id_type: 1 });
db.products.createIndex({ id_model: 1 });

db.product_variants.createIndex({ id_product: 1 });
db.product_variants.createIndex({ id_product: 1, color_name: 1, size_value: 1 });
db.product_variants.createIndex({ stock_quantity: 1 });
...
db.order_items.createIndex({ id_variant: 1 });
db.order_items.createIndex({ id_variant: 1, id_order: 1 });
